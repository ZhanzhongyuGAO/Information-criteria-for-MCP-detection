...
def generate_normal_ts(segments, m, means, stds):
    """
    Generate normal time series with variance change
    """
    n_segments = m + 1
...
def simulation(ic_list, margin, delta_std, min_size):
    """
    run simulation
    """
    ...
    for m in range(1, 21):
        ...
        for x in range(0, 1):
            ...
            for k in range(0, len(ic_list)):
                cp_detected = fc.detect_change_points(time_series, ic_list[k], min_size=min_size, para_nums=1)
...
if __name__ == "__main__":
    ...
    ordered_precisions, ordered_recalls, ordered_ratios = simulation(ic_list, margin, delta_std, min_size)
